...
class NeuralNetwork(nn.Module):
  ...
  def __init__(self, num_input, hidden_1, num_output):
      ...
      self.linear2 = myLinear(self.hidden_1,  self.num_output)
      
      self.activation = nn.LeakyReLU() # activation function of each hidden layer

  def forward(self, x):
      
      # Forward pass in Neural Net - Manually set in Pytorch format
      
      output1 = self.linear1(x)
      output1 = self.activation(output1)
      
      output2 = self.linear2(output1, x)

      return output2
  
    
# Training the Neural Net for the infrastructure data
class Trainer:
    
    def __init__( self, X, num_epoch     , num_train  , patience ,
    ...
    def train(self):
        ...
        for epoch in range(self.num_epoch):
            ...
            for j in range( int(X_train.shape[0] / self.mini_batch) ):
                ...
                train_loss    = loss_func(train_predict, X_current)  # Loss
            ...
            valid_loss    = loss_func(valid_predict, X_valid)
            ...
            if epoch % self.show_output == 0:
                ...
                print( "\n# " + str(epoch+1) + " Training Loss:    " + 
                                  str( float(format(train_loss, '.8f')) ) )
                
                print( "# "   + str(epoch+1) + " Validation Loss:  " + 
                                  str( float(format(valid_loss, '.8f')) ) )
    ...
    def test_data(self, X_test, accuracy):
        
        accuracy_index = []
        n, m = X_test.shape
        X_predict = self.model(X_test).detach()

        for i in range(n):
            
            counter = 0
            diff    = torch.abs( 21*( X_predict[i].reshape(-1,1) 
                                        - X_test[i, :].reshape(-1,1) ) )
            
            for j in range(m):
                
                if diff[j] <= accuracy and X_test[i,j].item() != 0:
                    counter += 1
                    
            total = np.count_nonzero(X_test[i, :])
